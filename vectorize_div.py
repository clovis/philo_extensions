...
class Indexer(object):
    ...
    def __init__(self, db, arrays=True, ranked_relevance=True, store_results=False):
        ...
        self.docs = glob(self.db_path + 'WORK/*tei.raw')
    ...
    def __init__sqlite(self):
        self.conn = sqlite3.connect(self.db_path + 'hits2_per_word.sqlite')
        self.c = self.conn.cursor()
        self.c.execute('''drop table if exists hits''')
        self.c.execute('''create table hits (word int, doc_id int, obj_id text, word_freq int, total_words int)''')
        self.c.execute('''create index word_index on hits(word)''')
               
    def index_docs(self, depth_level=0): ## depth level beyond doc id
        obj_count = 0
        for doc in self.docs:
            obj_count += 1
            doc_dict = {}
            endslice = 2 + depth_level
            ...
            for obj_id in doc_dict:
                ...
                for word_id in doc_dict[obj_id]:
                    ...
                    if self.r_r:
                        self.c.execute('insert into hits values (?,?,?,?,?)', (word_id, doc_id, obj_id, doc_dict[obj_id][word_id], sum_of_words))
